...
describe('summarizeFares', () => {
  it('one fare for a same-operator trip across a line change', () => {
    const s = summarizeFares([ride('KCI', 'C', 'KCI-BKS', 'KCI-MRI', 20000), ride('KCI', 'B', 'KCI-MRI', 'KCI-JAKK', 10000)])
  ...
  it('a walk transfer starts a new fare even within one operator', () => {
    const s = summarizeFares([ride('KCI', 'C', 'KCI-A', 'KCI-B', 10000), walk('KCI-B', 'KCI-C'), ride('KCI', 'T', 'KCI-C', 'KCI-D', 10000)])
  ...
  it('cross-operator trip sums both operators, walk distance counted in total distance only', () => {
    const s = summarizeFares([ride('KCI', 'C', 'KCI-BKS', 'KCI-SUDB', 22000), walk('KCI-SUDB', 'MRTJ-DKA', 400), ride('MRTJ', 'M', 'MRTJ-DKA', 'MRTJ-BHI', 800)])
  ...
  it('null segment fare nulls the total', () => {
    const s = summarizeFares([ride('MRTJ', 'M', 'MRTJ-ZZ', 'MRTJ-QQ', 1000)])
