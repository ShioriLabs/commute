 ...
 * not move for those consumers, which is why the two endpoints stay distinct
 * rather than one growing a mode flag. The app itself reads only the trip
 * endpoint.
 *
 * Distinct paths also mean distinct SWR keys, so neither surface can render the
 * other's cached body — which still matters, because a `/fares` body cached
 * before the app switched stays warm for its full life.
 */
