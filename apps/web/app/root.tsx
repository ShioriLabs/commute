        ...
        <InstallableProvider>
          <BootSplashDismissContext.Provider value={dismissSplash}>
            ...
            {/*
              ...
              arrives. z-boot-splash keeps it above everything that can exist during
              boot — including the map morph overlay, so a direct /map
              ...
              splash — and below the boot-fallback retry panel.
            ...
            */}
            {splashPhase !== 'gone' && (
              <div
                ref={splashRef}
                className={clsx(
                  'fixed inset-0 z-boot-splash flex items-center justify-center bg-rose-50/50 pointer-events-none',
      ...
      */}
      <div
        ...
        style={{
          position: 'fixed',
          inset: 0,
          // Above the boot splash (--z-index-boot-splash), which is still mounted
          // and 'visible' whenever the watchdog reveals this panel. A literal
          // rather than the token: this panel is the last resort when boot has
          // gone wrong, and that is exactly the case where the stylesheet
          // carrying the token may be the thing that failed to load.
          zIndex: 50,
          display: 'flex',
